...
import pickle
from run_gaussian import HierGaussian

"""
This is a grab bag of various tests and sanity checks I performed throughout the analysis 
to convince myself that things were copacetic. Some of them ended up in the paper but others
are just to make sure code worked correctly, that the statistics ideas I was making up 
made sense, or that I did math right.

NOTE: needs radvel v1.3.8 to load posteriors
"""


def importance_sample_linear():
    """
...
def PeK_sampling_to_Msini():
    ...
    Kmax_ms = 10_000  # [m/s]
    st_mass = 0.7
    Kmax = Kmax_ms / 28.4329 * st_mass ** (2 / 3) # [units of Jupiter semiamplitude times (stellar mass)^(2/3) (i.e. 1 means Mst^(2/3) * 28.4329 m/s)]
    print(Kmax)
...
def count_interloping_binaries():
    """"
...
def gaussian_sanity_check():
    """
    Check that I recover reasonable parameters for a mock Gaussian sample with well-constrained
    posteriors.
