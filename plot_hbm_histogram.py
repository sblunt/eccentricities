...
chains = chains.reshape((-1, n_e_bins, n_sma_bins, n_mass_bins))
...
ecc1_bin_smallmass = chains[:, 1, sma_bin_idx, msini_bin_idx]
ecc1_bin_bigmass = chains[:, 1, sma_bin_idx, msini_bin_idx+1]
...
smallmass_prob_of_moderate_ecc = ecc1_bin_smallmass / np.sum(chains[:, :, sma_bin_idx, msini_bin_idx], axis=1)
bigmass_prob_of_moderate_ecc = ecc1_bin_bigmass / np.sum(chains[:, :, sma_bin_idx, msini_bin_idx+1], axis=1)

relative_prob = smallmass_prob_of_moderate_ecc/bigmass_prob_of_moderate_ecc

cis = np.quantile(relative_prob, [0.05, 0.16,0.5,0.84,0.95])
print('A sub-Jovian planet around 1au has a ${:.1f}^{{+{:.1f}}}_{{-{:.1f}}}$x smaller chance of being moderately eccentric than a super-Jovian.'.format(cis[2], cis[3]-cis[2], cis[2]-cis[1]))

plt.figure()
plt.hist(relative_prob, bins=20, range=(0,5), color='grey',alpha=0.5, density=True)
